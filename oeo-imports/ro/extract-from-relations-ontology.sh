...
robot merge --input ${tmpdir}/ro-extracted-w-hierarchy.owl --input ${tmpdir}/ro-extracted-n-hierarchy.owl annotate --ontology-iri https://openenergyplatform.org/ontology/oeo/imports/ro-extracted.owl --version-iri https://openenergyplatform.org/ontology/oeo/dev/imports/ro-extracted.owl --annotation rdfs:comment "This file contains externally imported content from the Relations Ontology (RO) for import into the Open Energy Ontology (OEO). It is automatically extracted using ROBOT from the list of selected terms (ro-extract-w-hierarchy.txt, ro-extract-n-hierarchy.txt) located in the OEO-repository." --output ${tmpdir}/ro-extracted.owl
